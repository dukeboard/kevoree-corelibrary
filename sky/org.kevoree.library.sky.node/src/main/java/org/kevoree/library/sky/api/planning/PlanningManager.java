...
public class PlanningManager extends KevoreeKompareBean {
    ...
    private void processStopInstance(Instance actualInstance, AdaptationModel adaptationModel, ContainerRoot actualRoot) {
        ...
        ccmd2.setPrimitiveType(actualRoot.findAdaptationPrimitiveTypesByID(JavaPrimitive.StopInstance));
    ...
    private void processStartInstance(Instance updatedInstance, AdaptationModel adaptationModel, ContainerRoot updateRoot) {
        ...
        AdaptationPrimitive ccmd2 = getAdaptationModelFactory().createAdaptationPrimitive();
        ccmd2.setPrimitiveType(updateRoot.findAdaptationPrimitiveTypesByID(JavaPrimitive.StartInstance));
    ...
    public AdaptationModel schedule(AdaptationModel adaptationModel, String nodeName) {
        if (!adaptationModel.getAdaptations().isEmpty()) {
            ...
            for (AdaptationPrimitive primitive : adaptationModel.getAdaptations()) {
                if (primitive.getPrimitiveType().getName().equals(JavaPrimitive.StopInstance) && primitive.getRef() instanceof ContainerNode) {
                    primitives.add(primitive);
                }
            }
            createNextStep(JavaPrimitive.StopInstance, primitives);
            ...
            primitives.clear();
            for (AdaptationPrimitive primitive : adaptationModel.getAdaptations()) {
                if (primitive.getPrimitiveType().getName().equals(JavaPrimitive.RemoveBinding) ||
                        primitive.getPrimitiveType().getName().equals(JavaPrimitive.RemoveFragmentBinding)) {
                    primitives.add(primitive);
                }
            }
            createNextStep(JavaPrimitive.RemoveBinding, primitives);

            // REMOVE INSTANCE
            primitives.clear();
            for (AdaptationPrimitive primitive : adaptationModel.getAdaptations()) {
                if (primitive.getPrimitiveType().getName().equals(JavaPrimitive.RemoveInstance)) {
                    primitives.add(primitive);
                }
            }
            createNextStep(JavaPrimitive.RemoveInstance, primitives);

            // REMOVE TYPE
            primitives.clear();
            for (AdaptationPrimitive primitive : adaptationModel.getAdaptations()) {
                if (primitive.getPrimitiveType().getName().equals(JavaPrimitive.RemoveType)) {
                    primitives.add(primitive);
                }
            }
            createNextStep(JavaPrimitive.RemoveType, primitives);

            // REMOVE TYPE
            primitives.clear();
            for (AdaptationPrimitive primitive : adaptationModel.getAdaptations()) {
                if (primitive.getPrimitiveType().getName().equals(JavaPrimitive.RemoveDeployUnit)) {
                    primitives.add(primitive);
                }
            }
